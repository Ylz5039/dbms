...
import com.parser.ast.expression.primary.function.info.CurrentUser;
...
import com.parser.ast.fragment.SortOrder;
import com.parser.ast.fragment.ddl.ColumnDefinition;
import com.parser.ast.fragment.ddl.ColumnDefinition.SpecialIndex;
import com.parser.ast.fragment.ddl.ColumnDefinition.Storage;
import com.parser.ast.fragment.ddl.TableOptions;
import com.parser.ast.fragment.ddl.TableOptions.Compression;
...
import com.parser.ast.fragment.ddl.index.IndexDefinition.IndexType;
import com.parser.ast.fragment.ddl.index.IndexOption;
import com.parser.ast.stmt.SQLStatement;
import com.parser.ast.stmt.compound.condition.Characteristics;
import com.parser.ast.stmt.compound.condition.Characteristics.Characteristic;
import com.parser.ast.stmt.ddl.*;
import com.parser.ast.stmt.ddl.DDLAlterTableStatement.ReNameIndex;
import com.parser.ast.stmt.ddl.DDLAlterTableStatement.WithValidation;
import com.parser.ast.stmt.ddl.DDLCreateProcedureStatement.ProcParameterType;
import com.parser.ast.stmt.ddl.DDLCreateTableStatement.ForeignKeyDefinition;
import com.parser.ast.stmt.ddl.DDLCreateTriggerStatement.TriggerEvent;
import com.parser.ast.stmt.ddl.DDLCreateTriggerStatement.TriggerOrder;
import com.parser.ast.stmt.ddl.DDLCreateTriggerStatement.TriggerTime;
import com.parser.ast.stmt.dml.DMLQueryStatement;
import com.parser.ast.stmt.extension.DropPrepareStatement;
import com.parser.ast.stmt.extension.ExtDDLCreatePolicy;
import com.parser.ast.stmt.extension.ExtDDLDropPolicy;
...
/**
    ...
    public DDLStatement ddlStmt() throws SQLSyntaxErrorException, IOException {
        Identifier idTemp1;
        Identifier idTemp2;
        ...
        switch (lexer.token()) {
            ...
            case KW_CREATE:
                ...
                switch (lexer.token()) {
                    ...
                    case KW_DATABASE:
                        ...
                        lexer.nextToken();
                        boolean ifNotExists1 = ifNotExists();
                        Identifier database = identifier();
                        return createDatabase(false,ifNotExists1, database);
                    case IDENTIFIER:
            ...
            case KW_USE:
                lexer.nextToken();
                Identifier database = identifier();
                return new DDLUseDatabaseStatement(database);
            default:
    ...
    private void databaseOptions(List<DatabaseOptions> options) throws SQLSyntaxErrorException {
        while (true) {
            switch (lexer.token()) {
                case KW_CHARACTER:
                    lexer.nextToken(); // Skip 'CHARACTER'
                    if (lexer.token() == MySQLToken.KW_SET) {
                        lexer.nextToken(); // Skip 'SET'
                        Identifier charset = identifier(); // Get character set identifier
                    } else {
                        throw new SQLSyntaxErrorException("Expected 'SET' after 'CHARACTER'");
                    }
                    break;
/*                case KW_COLLATE:
                    lexer.nextToken(); // Skip 'COLLATE'
                    Identifier collation = identifier(); // Get collation identifier
                    break;*/
                case EOF:
                    return; // End of options
                default:
                    return; // End of option parsing
            }
        }
    }


    private DDLStatement createDatabase(boolean temp, boolean ifNotExists, Identifier database)
            throws SQLSyntaxErrorException {

        DDLCreateDatabaseStatement stmt = new DDLCreateDatabaseStatement(ifNotExists, database, null);

        List<DatabaseOptions> options = new ArrayList<>();

        databaseOptions(options);
        stmt = new DDLCreateDatabaseStatement(ifNotExists, database, options);
        ...
        switch (lexer.token()) {
            ...
            case KW_COLLATE:
                lexer.nextToken();
                Identifier collation = identifier();
                break;
            case EOF:
                return stmt;
            default:
                throw new SQLSyntaxErrorException(
                        "DDL CREATE DATABASE statement not end properly");
        }
